...
//A través de tres prompts, pídale al usuario los siguientes datos.

/////////// PARA EL CASO LOGIN

// TODOS LOS PUNTOS TIENEN QUE HACERSE CON FUNCIONES. CADA PUNTO PUEDE EXPRESARSE COMO UNA FUNCIÓN. 
//LAS RESPONSABILIDADES DEBEN ESTAR SEPARADAS.
//UTILICE FUNCIONES QUE RECIBAN PARÁMETROS Y PASE ARGUMENTOS EN LA LLAMADA.


// a) A través de 3 prompts, pídale al usuario que ingrese su nombre, usuario y su contraseña. 
//Valide que el usuario y la contraseña coincidan con los datos de las variables, estos datos tienen que ser iguales entre sí.
// En el caso de que no lo sean, devuelva un mensaje en forma de alerta, para cada uno de los casos: 
//"Su usuario es erróneo" o "Su contraseña es errónea". EN EL CASO DE QUE SE EQUIVOQQUE EN LOS DOS: "SU USUARIO Y CONTRASEÑA SO ERRÓNEOS".
// b) Luego de que el usuario se loguee exitosamente, envíe una alerta que notifique "Bienvenido ${nombre}". 
//La alerta debe saludar al usuario que se loguea.
// c) Valide con un bucle que el usuario tiene una arroba. De ser así, envíe el mensaje en consola: 
//"Es un mail válido" e interrumpa el ciclo for con break. 
// d) Valide si la contraseña del usuario tiene más de 8 caracteres. En el caso de que tenga más de ocho caracteres, no haga nada.
// De lo contrario, envíe una alerta con el siguiente mensaje: "Su contraseña es insegura."
// e) Valide con un bucle que la contraseña al menos tenga un número. En el caso de que no tenga uno, 
//envíe un prompt que exprese lo siguiente: "Su contraseña debe tener caracteres alfanuméricos. Desea cambiarla?".
// Si el usuario dice si, entonces cambie el valor de la contraseña.

const usuarioConEmail= "rdurbano@outlook.com"
const contrasenia= "javasc"

function validardatos (){
const nombre = prompt("ingrese su nombre")
const usuario = prompt("ingrese su usuario")
const password = prompt("ingrese su contrasenia")
if(usuario === usuarioConEmail && password === contrasenia){
    alert("Bienvenido a su sesion "  + nombre )
} if (usuario === usuarioConEmail && password != contrasenia){
...
const contraalfanum = () =>{
    for (let i=0; i < contrasenia.length; i++){ 
    if(contrasenia[i].includes("numero"));
}console.log=confirm("Su contraseña debe tener caracteres alfanuméricos. Desea cambiarla?");{
    const nuevacontra =prompt("la nueva contraseña debe ser alfanumerica y contener mas de 8 letras ");{
        if(nuevacontra.includes("numeros"));{
            alert("su contraseña ahora es segura")
        }
    }
}
}  
